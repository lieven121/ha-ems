...
function slotAction(slot) {
  ...
  // Support both old data format (battery_manual_override_w) and new (action field)
...
function slotEndTimeStr(slots, idx) {
  ...
  const dur = idx > 0 ? (d - new Date(slots[idx - 1].start ?? slots[idx - 1].time)) : 3600000;
...
class EmsDashboardCard extends HTMLElement {
  ...
  _buildSlots(hass) {
    ...
    // Build base slots from price entity
    let slots = [];
    if (pCfg?.entity) {
    ...
    // Merge planning data from EMS planning sensor if configured
    ...
    if (planEntity && slots.length > 0) {
      ...
      if (planList.length > 0) {
        // Normalize to ms timestamp to handle Python str(datetime) vs ISO format differences
        // e.g. "2026-04-21 22:00:00+02:00" vs "2026-04-21T22:00:00+02:00"
        const planMap = {};
  ...
  _render() {
      ...
      if (planEntity) {
        ...
        if (planState) {
          ...
          if (Array.isArray(planList) && planList.length > 0) {
            ...
            // Show key comparison to diagnose merge matching
            if (slots.length > 0) {
    ...
    const showBattery = this._config.layout?.show_battery !== false && !!this._config.integration?.battery_entity;
    if (battSvg) {
      if (showBattery && n > 0) {
        const battEntity = this._config.integration.battery_entity;
        const battState  = this._hass?.states[battEntity];
        const currentPct = battState ? parseFloat(battState.state) : null;
        const lineColor  = this._config.integration?.battery_line_color
          ? `rgb(${this._config.integration.battery_line_color.r},${this._config.integration.battery_line_color.g},${this._config.integration.battery_line_color.b})`
          : '#06b6d4';

        // viewBox maps x: 0–100 (slot units) and y: 0–CHART_H (px)
        // This avoids % in path d which SVG does not support
        ...
        // Predicted dashed line (dots align to bar centres)
        ...
        // Current battery horizontal solid line
  ...
  _renderPopup(i) {
    ...
    const ns    = this._nowSlotIdx();
    const act   = slotAction(slot);
    const entryId  = this._config.integration?.entry_id;
    const deviceId = this._config.integration?.device_id;
    ...
    // kWp default wattage (stored as W in entity, entity state is in kW)
    const kwpW = Math.round((parseFloat(this._hass?.states[this._kwpEntityId]?.state) || 3) * 1000);
    ...
    this.shadowRoot.getElementById('popup').innerHTML = `
      ...
      ${this._config.layout?.show_actions !== false ? editHtml : ''}`;
    ...
    if (deviceId || entryId) {
      ...
      solarToggleIds.forEach(id => {
        ...
        if (btn) btn.addEventListener('click', e => { e.stopPropagation(); btn.classList.toggle('on'); });
      });
      ...
      ['wc-car-net', 'wc-car-bat'].forEach(id => {
        ...
        card.addEventListener('click', e => {
          e.stopPropagation();
          // Only toggle if click is NOT on an input inside the card
          if (e.target.tagName === 'INPUT') return;
          card.classList.toggle('on');
        });
        // Stop propagation from inputs inside the card so they don't trigger toggle
      ...
      sr.getElementById('action-apply')?.addEventListener('click', e => {
        ...
        let svcPromise;
        if (selAction === 'idle') {
          svcPromise = this._hass.callService('ha_ems', 'planning_idle', commonData);
          ...
          svcPromise = this._hass.callService('ha_ems', 'planning_charge',
          ...
          const untilPct = untilRaw ? parseFloat(untilRaw) : null;
          svcPromise = this._hass.callService('ha_ems', 'planning_discharge',
            { ...commonData, wattage: watts, ...(untilPct != null ? { until_pct: untilPct } : {}) });
          ...
          svcPromise = this._hass.callService('ha_ems', 'planning_use_net',
          ...
          const netW       = netCardOn ? (parseFloat(sr.getElementById('param-car-net-w')?.value) || 0) : 0;
          const batW       = batCardOn ? (parseFloat(sr.getElementById('param-car-bat-w')?.value) || 0) : 0;
          const batUntilR  = sr.getElementById('param-car-bat-until')?.value;
          const batUntil   = (batCardOn && batUntilR) ? parseFloat(batUntilR) : null;
          svcPromise = this._hass.callService('ha_ems', 'planning_car_charge_slot', {
            ...commonData,
            use_solar: useSolar,
            use_net_wattage:     netW,
            use_battery_wattage: batW,
